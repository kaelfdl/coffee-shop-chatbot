...
class RecommendationAgent():
    ...
    def recommendation_classification(self, message):
        ...
        3. Popular Recommendations by Category: Here the user asks to recommend them a product in a category. Like what coffee to you recommend me to get? We recommend items that are popular in the category of the user's requested category.
        ...
        Your output should be in a structured json format like so. Each key is a string, and each value is a string. Make sure to follew the format exactly:
        {
            ...
            "parameters": This is a python list. It's either a list of items for apriori recommendations or a list of categories for popular by category recommendations. Leave it empty for popular recommendations. Make sure to use the exact strings from the list of items and categories above. 
        }
    ...
    def get_recommendations_from_order(self, messages, order):
        ...
        Your task is to recommend items to the user based on their input message. And respond in a friendly but concise way. And put it in an unordered list with a very short description.
